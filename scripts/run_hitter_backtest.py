#!/usr/bin/env python
"""
Run walk-forward backtest for all hitter stats.

Rate stats: K%, BB%, HR/PA, xwOBA (Bayesian vs Marcel, ensemble, CRPS, PPC)
Counting stats: Total Ks, Total BBs, Total HRs, Total SBs
...
-----
    ...
    # Skip counting stats
    python scripts/run_hitter_backtest.py --quick --skip-counting
...
def main() -> None:
    ...
    summary = run_hitter_backtest(stats=stats, folds=RATE_FOLDS, **sampling)

    print("\n=== HITTER RATE STAT BACKTEST RESULTS ===")
    ...
    if not args.skip_counting and args.stat is None:
        from src.evaluation.counting_backtest import run_hitter_counting_backtest
        ...
        counting_summary = run_hitter_counting_backtest(
            folds=COUNTING_FOLDS, **counting_sampling,
        )

        print("\n=== HITTER COUNTING STAT BACKTEST RESULTS ===")
        display_cols = [
