...
# Marcel-style recency weights (most-recent first)
...
def _classify_pitcher_roles(
    seasons: list[int],
    from_season: int,
) -> pd.DataFrame:
    """Classify reliever roles and save to parquet.

    Returns
    -------
    pd.DataFrame
        Reliever roles (pitcher_id, role, ...).
    """
...
def _build_pitcher_posteriors(
    pitcher_results: dict,
    active_pitchers: pd.DataFrame,
    pitcher_ext: pd.DataFrame,
    from_season: int,
) -> dict[int, dict[str, np.ndarray]]:
    """Build posterior rate sample dicts for each active pitcher.

    Returns
    -------
    dict[int, dict[str, np.ndarray]]
        Mapping pitcher_id -> {k_rate, bb_rate, hr_rate} arrays.
    """
    ...
    stat_configs = [
        ("k_rate", "k_rate", (5, 20)),
        ("bb_rate", "bb_rate", (3, 30)),
        ("hr_per_bf", "hr_rate", None),  # HR uses special fallback
    ]
    ...
    for _, prow in active_pitchers.iterrows():
        ...
        for result_key, rate_key, fallback_params in stat_configs:
            samples = pitcher_results.get(result_key, {}).get("rate_samples", {})
            if pid in samples:
                rates[rate_key] = samples[pid].copy()
            elif fallback_params is not None:
                rates[rate_key] = rng.beta(*fallback_params, size=8000)
            else:
...
def _compute_weighted_pitches_per_start(
    starter_ids: set[int],
    from_season: int,
) -> dict[int, float]:
    """Query per-starter weighted average pitches from recent seasons.

    Returns
    -------
    dict[int, float]
        Mapping pitcher_id -> regressed avg pitches per start.
    """
    from src.data.db import read_sql as _read_pitches

    _POP_AVG_PITCHES = 88.0

    pitch_counts = _read_pitches("""
    ...
    pitch_lookup: dict[int, float] = {}
    if pitch_counts.empty:
        return pitch_lookup

    for pid in starter_ids:
        ph = pitch_counts[
            pitch_counts["pitcher_id"] == pid
        ].sort_values("season", ascending=False).head(3)
        if ph.empty:
            continue
        wts = list(MARCEL_WEIGHTS[:len(ph)])
        wtd = sum(
            a * s * w
        ...
        pitch_lookup[pid] = rel * wtd + (1 - rel) * _POP_AVG_PITCHES

    logger.info(
        "Pitcher avg pitches: %d starters, range %.0f-%.0f (pop=%.0f)",
        len(pitch_lookup),
        min(pitch_lookup.values()) if pitch_lookup else 0,
        max(pitch_lookup.values()) if pitch_lookup else 0,
        _POP_AVG_PITCHES,
    )
    return pitch_lookup
...
def _build_starter_priors(
    ...
    """Build Marcel-style starter priors (n_starts, avg_pitches) with COVID adj.

    Returns
    -------
    pd.DataFrame or None
        Starter prior rows, or None if no starters found.
    """
    _POP_AVG_PITCHES = 88.0
    ...
    for _, prow in active_pitchers.iterrows():
        ...
        if player_hist.empty:
            starter_prior_rows.append({
                ...
                "n_starts_mu": 30.0,
                "n_starts_sigma": 6.0,
                "avg_pitches": pitch_lookup.get(pid, _POP_AVG_PITCHES),
            })
            continue

        # Weighted starts (5/4/3)
        weights = list(MARCEL_WEIGHTS[:len(player_hist)])
    ...
    return pd.DataFrame(starter_prior_rows) if starter_prior_rows else None
...
def run_sim_pitcher(
    *,
    ...
    try:
        ...
        reliever_roles = _classify_pitcher_roles(seasons, from_season)
        ...
        pitcher_posteriors = _build_pitcher_posteriors(
            pitcher_results, active_pitchers, pitcher_ext, from_season,
        )

        starter_ids = set(
            active_pitchers[active_pitchers["is_starter"] == 1]["pitcher_id"]
        )
        pitch_lookup = _compute_weighted_pitches_per_start(starter_ids, from_season)
        starter_priors_df = _build_starter_priors(
            active_pitchers, pitcher_ext, starter_ids, pitch_lookup, from_season,
        )
...
def _build_hitter_posteriors(
    hitter_results: dict,
    active_hitters: pd.DataFrame,
    hitter_ext: pd.DataFrame,
    from_season: int,
) -> dict[int, dict[str, np.ndarray]]:
    """Build posterior rate sample dicts (K%, BB%, HR%) for each active hitter.

    Returns
    -------
    dict[int, dict[str, np.ndarray]]
        Mapping batter_id -> {k_rate, bb_rate, hr_rate} arrays.
    """
    logger.info("Building hitter posterior rate samples...")
    posteriors: dict[int, dict[str, np.ndarray]] = {}
    rng = np.random.default_rng(42)

    fallback_params = {"k_rate": (5, 18), "bb_rate": (3, 30)}
    ...
    for _, hrow in active_hitters.iterrows():
        ...
        for stat_key in ["k_rate", "bb_rate"]:
            samples = hitter_results.get(stat_key, {}).get("rate_samples", {})
            if bid in samples:
                rates[stat_key] = samples[bid].copy()
            else:
                rates[stat_key] = rng.beta(*fallback_params[stat_key], size=8000)

        rates["hr_rate"] = _build_hr_beta_posterior(
            bid, hitter_ext, from_season, rng,
        )
        posteriors[bid] = rates

    logger.info("Built hitter posteriors for %d batters", len(posteriors))
    return posteriors


def _build_hr_beta_posterior(
    batter_id: int,
    hitter_ext: pd.DataFrame,
    from_season: int,
    rng: np.random.Generator,
) -> np.ndarray:
    """Build a Marcel-weighted Beta posterior for HR rate.

    Returns
    -------
    np.ndarray
        8000 posterior samples of HR/PA rate.
    """
    bid_hist = hitter_ext[
        (hitter_ext["batter_id"] == batter_id)
    ...
    return rng.beta(1, 30, size=8000).astype(np.float32)


def _compute_sb_rates(
    active_hitters: pd.DataFrame,
) -> dict[int, tuple[float, float]]:
    """Compute SB rates blending observed SB/game with sprint speed.

    Returns
    -------
    dict[int, tuple[float, float]]
        Mapping batter_id -> (mean_rate, sigma).
    """
...
def _compute_babip_adjustments(
    hitter_ext: pd.DataFrame,
    from_season: int,
) -> dict[int, float]:
    """Compute reliability-weighted BABIP adjustments from recent seasons.

    Returns
    -------
    dict[int, float]
        Mapping batter_id -> BABIP adjustment (positive = above league avg).
    """
...
def _build_bip_profiles(
    hitter_ext: pd.DataFrame,
    hitter_posterior_ids: set[int],
    from_season: int,
) -> dict[int, np.ndarray]:
    """Build per-player BIP outcome profiles from quality metrics and observed splits.

    Parameters
    ----------
    hitter_ext : pd.DataFrame
        Multi-season hitter extended data.
    hitter_posterior_ids : set[int]
        Set of batter_ids that have posteriors (filter for relevance).
    from_season : int
        Current/projection season.

    Returns
    -------
    dict[int, np.ndarray]
        Mapping batter_id -> BIP probability array [out, single, double, triple].
    """
    ...
    try:
        ...
        for _, row in quality_agg.iterrows():
            ...
            if bid not in hitter_posterior_ids:
                continue
...
def run_sim_hitter(
    *,
    ...
    try:
        ...
        hitter_posteriors = _build_hitter_posteriors(
            hitter_results, active_hitters, hitter_ext, from_season,
        )
        sb_rates = _compute_sb_rates(active_hitters)
        ...
        hitter_babip = _compute_babip_adjustments(hitter_ext, from_season)
        bip_profiles = _build_bip_profiles(
            hitter_ext, set(hitter_posteriors.keys()), from_season,
        )
